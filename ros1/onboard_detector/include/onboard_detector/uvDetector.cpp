...
namespace onboardDetector{
    ...
    // 用于从深度图（depth map）中提取 ​​U-map​​（一种垂直方向的深度分布直方图）
    ...
    void UVdetector::detect()
    {
        ...
        // ​​输入​​：this->U_map（垂直深度分布直方图）
        // ​​输出​​：this->bounding_box_U（在U-map坐标系中的物体边界框）
        // 核心任务​​：检测U-map中的连续高密度区域，并将其合并为物体框
        this->extract_bb();

        // extract bounding box
        // 通过几何变换，将图像坐标转换为真实世界坐标（鸟瞰视角）
        /* U-map中的每个边界框包含：
​           ​水平位置（x）​​：图像列坐标
​           ​深度信息（y）​​：U-map行坐标（代表深度区间）
        */
