...
// number_data：数据点数量
// data：数据点数组，每个点是dimension_data维向量
double KMeans::Cluster(int number_data, double **data){
	double movements_centroids = 0;

	int *label = new int[number_data]; // 为每个点分配标签

	double *mean = new double[dimension_data];

	for (int i = 0; i < number_data; i++){
		label[i] = Classify(data[i]); // 找到最近的簇中心
	}

	//对每个簇（共 number_clusters个）
	for (int j = 0; j < number_clusters; j++){
		int number_sample = 0; // 计数器：统计当前簇中的点数

		double movements = 0;  // 当前簇中心的总移动距离

		for (int k = 0; k < dimension_data; k++){
			mean[k] = 0;  // 初始化均值向量（新质心）
		}
		for (int i = 0; i < number_data; i++){
			if (label[i] == j){   // 如果点i属于当前簇j
				for (int k = 0; k < dimension_data; k++){
					mean[k] += data[i][k];
				}
				number_sample++; // 增加计数
		...
		for (int k = 0; k < dimension_data; k++){
			...
			centroid[j][k] = mean[k]; //质心（Centroid）​
		} //如果所有中心的移动距离总和 movements_centroids接近0，说明聚类已稳定，可提前终止迭代
