...
void obstacleClustering::run(const std::vector<Eigen::Vector3d>& localCloud){
	...
	std::vector<pointCluster> refinedCluster = this->initialClusters_; //当前层级的聚类列表
	std::vector<bool> refineComplete (int(refinedCluster.size()), false); //标记每个聚类是否已完成细化
	
	// 核心循环：多级细化
	// 控制细化的最大深度（treeLevel_ = 3）
...
void obstacleClustering::runKmeans(const pointCluster& cluster, std::vector<pointCluster>& cloudClusters){
	int dimension = 3; // 3D空间
	int clusterNum = 2; // 分割为2个聚类
	...
	for (int i=0; i<int(cluster.points.size()); ++i){
		...
		if (dist > maxDist){
			maxDist = dist;
			fpoint = cluster.points[i];// 找到距离质心最远的点
		}
	...
	for (int i=0; i<int(cluster.points.size()); ++i){
		...
		if (dist > maxDist){
			maxDist = dist;
			ffpoint = cluster.points[i];// 找到距离第一个点最远的点
		}
	}
	std::vector<Eigen::Vector3d> initPoints {fpoint, ffpoint}; //使用这两个点作为初始中心
	...
	for (int i=0; i<int(cloudClusters.size()); ++i){
		...
		cloudClusters[i].centroid = (clusterMin + clusterMax)/2.0;//计算中心 = (min + max)/2
...
double obstacleClustering::getOrientation(const pointCluster& cluster, bboxVertex& vertex){
	double maxDensity = 0.0;  //  记录最大密度
	double bestAngle = 0.0;  // 记录最优角度
	bboxVertex bestVertex;    // 记录最优边界框

	// angleDiscreteNum_ = 20：在0-180°范围内测试20个角度
