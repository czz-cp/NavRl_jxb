...
namespace mapManager{
	...
	void occMap::registerCallback(){
		if (this->sensorInputMode_ == 0){
			// depth pose callback
			//通过 sensorInputMode_决定使用哪种传感器数据：
			//1. 0：深度图模式 (depthTopicName_)
			//2. 1：点云模式 (pointcloudTopicName_)
			...
			if (this->localizationMode_ == 0){
				...
				// 同步深度图和位姿数据，使用ApproximateTime同步策略
				...
				// 同步深度图和里程计数据，使用ApproximateTime同步策略
	...
	void occMap::staticClusteringCB(const ros::TimerEvent& ){
		...
		this->currCloud_ = currCloud; //保存当前检测到的障碍物点云
	...
	void occMap::raycastUpdate(){
		...
		xmin = xmax = this->position_(0); // 初始化为相机位置
		ymin = ymax = this->position_(1); //​ ​用途​​：跟踪所有射线终点的边界，确定需要更新的局部区域
		...
		for (int i=0; i<this->projPointsNum_; ++i){
			...
			// rayPoint是体素的整数坐标（比如(2,3,4)）
			while (this->raycaster_.step(rayPoint)){
				// 所以加0.5是为了从体素索引转换到体素中心坐标
		...
		// 为什么使用对数几率 贝叶斯更新简化：乘法变加法 加法运算比乘法快
		// 更新规则:
		//L_new = L_old + log(p_hit / (1 - p_hit))  // 如果命中
		//L_new = L_old + log(p_miss / (1 - p_miss)) // 如果未命中
		...
		while (not this->updateVoxelCache_.empty()){
			...
			hit = this->countHit_[cacheAddress];  // 命中次数（检测到障碍物）
			miss = this->countHitMiss_[cacheAddress] - hit; // 未命中次数（自由空间

			if (hit >= miss and hit != 0){
				logUpdateValue = this->pHitLog_; // 倾向于标记为占据
			}
			else{
				logUpdateValue = this->pMissLog_; // 倾向于标记为自由
			}
	...
	void occMap::inflateLocalMap(){
		...
		// ceil(robotSize/(2*mapRes))：计算需要膨胀的体素数量
		int xInflateSize = ceil(this->robotSize_(0)/(2*this->mapRes_));
		int yInflateSize = ceil(this->robotSize_(1)/(2*this->mapRes_));
		int zInflateSize = ceil(this->robotSize_(2)/(2*this->mapRes_));
