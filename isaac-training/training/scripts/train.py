...
def main(cfg):
    ...
    # 统计收集器设置
    # 从观察空间中提取所有以"stats"开头的键（如回合奖励、长度等）
    episode_stats_keys = [
    ...
    # RL Data Collector
    # frames_per_batch：每批收集的帧数 = 环境数量 × 每环境batch_size数
    # total_frames：总训练帧数
    # return_same_td=True：内存复用，提高效率
    # exploration_type=RANDOM：使用随机探索策略
    ...
    for i, data in enumerate(collector):
        ...
        episode_stats.add(data) # 从数据中提取并更新回合统计信息
        
        # len(episode_stats)：当前统计收集器中存储的已完成回合数量
        # transformed_env.num_envs：并行环境的总数量
        # 当所有环境至少完成一个回合时，进行日志记录
