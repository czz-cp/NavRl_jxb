...
class PPO(TensorDictModuleBase):
    def __init__(self, cfg, observation_spec, action_spec, device):
        ...
        self.actor = ProbabilisticActor(
            ...
            #PPO算法的核心需求 存储采样动作的对数概率，用于重要性采样计算
    ...
    def train(self, tensordict):
        # tensordict可以存储和操作嵌套的张量数据
        # tensordict: (num_env, num_frames, dim), batchsize = num_env * num_frames
        next_tensordict = tensordict["next"]
        # 禁用梯度计算，因为这是目标值计算，不应该影响网络参数
        with torch.no_grad():
            # 向量化映射，高效地对批量数据应用特征提取器
        ...
        # 价值目标计算和优势估计
        # 策略关心的是相对改进，价值关心的是绝对预测。
        adv, ret = self.gae(rewards, dones, values, next_values)
        adv_mean = adv.mean()
        adv_std = adv.std() 
        adv = (adv - adv_mean) / adv_std.clip(1e-7) # normalize advantage
        ...
        # 将列表中的多个统计字典合并为一个 TensorDict
        infos = torch.stack(infos).to_tensordict()
        
        # 对所有统计值（如loss、KL散度）​​沿第0维求平均​
    ...
    def _update(self, tensordict): # tensordict shape (batch_size, )
        ...
        # 计算对数概率​​：评估当前策略下实际执行动作的概率（用于PPO的比率计算）
        ...
        # Critic Loss 
        b_value = tensordict["state_value"] # 旧价值估计 (baseline value)
        ret = tensordict["ret"] # 归一化后的真实回报目标
        value = self.critic(tensordict)["state_value"]  # 新价值估计
        ...
        # 梯度裁剪
        # 当梯度向量的范数超过max_norm=5时，将整个梯度向量按比例缩小
