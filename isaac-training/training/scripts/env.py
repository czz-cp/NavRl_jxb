...
class NavigationEnv(IsaacEnv):
    ...
    def __init__(self, cfg):
        ...
        # LiDAR params:
        self.lidar_range = cfg.sensor.lidar_range #表示LiDAR最远可以探测10米处的障碍物
        # 设置LiDAR的垂直视场角(FOV)，并限制在-89°到89°之间
        self.lidar_vfov = (max(-89., cfg.sensor.lidar_vfov[0]), min(89., cfg.sensor.lidar_vfov[1]))
        # 设置LiDAR的垂直方向上的光束数量，这里设置为4个
        self.lidar_vbeams = cfg.sensor.lidar_vbeams

        self.lidar_hres = cfg.sensor.lidar_hres #设置LiDAR的水平分辨率，这里设置为10度

        # 计算LiDAR的水平方向上的光束数量，这里设置为360/10=36个
        self.lidar_hbeams = int(360/self.lidar_hres)

        super().__init__(cfg, cfg.headless) #cfg.headless: 是否以无头模式运行（不显示图形界面）
        ...
        # LiDAR Intialization
        # LiDAR传感器的初始化配置，包括激光束的配置和传感器的参数设置
    ...
    def _design_scene(self):
        ...
        #创建了复杂的地形环境，包含随机生成的障碍物
        self.map_range = [20.0, 20.0, 4.5]

        terrain_cfg = TerrainImporterCfg(
            num_envs=self.num_envs,#为所有环境生成地形
            env_spacing=0.0,#环境间无间距（共享同一地形）
            prim_path="/World/ground",#地形生成路径
    ...
    def _compute_state_and_obs(self):
        ...
        #  LiDAR数据处理，计算LiDAR的扫描数据，即LiDAR能够探测到的距离
        # self.lidar.data.ray_hits_w:所有射线命中点的世界坐标，(num_envs, num_rays, 3)
        # total_rays = lidar_hbeams × lidar_vbeams = 36 × 4 = 144
        ...
        vel_w = self.root_state[..., :170] # world vel
